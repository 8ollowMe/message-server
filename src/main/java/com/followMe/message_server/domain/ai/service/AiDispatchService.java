...
public class AiDispatchService {
  ...
  public AiDispatchResult calculateAndSave(DispatchDeadlineProcessRequest request) {
    ...
    try {
      ...
      try {
        AiHttpResponse rawResponse =
            aiClient.requestDispatchDeadline(AiHttpRequest.builder().prompt(prompt).build());
        ...
        return AiDispatchResult.builder().aiHistory(history).dispatchDeadlineResult(result).build();
