...
public class SlackMessageServiceImpl implements SlackMessageService {
  ...
  public SlackMessage sendOrderAlert(
      ...
      UUID requestedBy) {
    UUID auditor =
        requestedBy != null ? requestedBy : UUID.fromString("00000000-0000-0000-0000-000000000000");
    ...
    try {
      ...
      SlackMessage slackMessage =
          SlackMessage.create(
              MessageType.ORDER_ALERT, userId, ReferenceType.ORDER, referenceId, formattedMessage);
  ...
  public PageResponse<SlackMessageSummaryResponse> getList(PageRequest pageRequest) {
    Page<SlackMessage> page =
        slackMessageRepository.findAllByDeletedAtIsNull(
