...
public class HttpAiClient implements AiClient {
  ...
  public AiHttpResponse requestDispatchDeadline(AiHttpRequest request) {
    try {
      ...
      return AiHttpResponse.builder().rawResponse(response).build();

    } catch (WebClientResponseException e) {
      log.error("AI 호출 실패 status={}, body={}", e.getStatusCode(), e.getResponseBodyAsString(), e);
  ...
  @Override
  public DispatchDeadlineResponse generateDispatchDeadline(
      String internalRequest, String systemId, DispatchDeadlineRequest request) {
    return null;
  }
}
